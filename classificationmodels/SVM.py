...
trainY_upsampled = data_upsampled2['severity_degree']
trainX_upsampled = data_upsampled2.drop('severity_degree', axis=1)
...
model = svm.SVC(random_state=44)
parameters = [{'C': [1,20,50], 'kernel':['linear', 'poly', 'rbf'], 'gamma':[0.1 , 0.01, 0.001]}]


grid_svc = GridSearchCV(model, parameters, cv=10, scoring='f1_macro')
grid_svc.fit(trainX_upsampled, trainY_upsampled)
print(grid_svc.best_params_)
# {'C': 50, 'gamma': 0.1, 'kernel': 'linear'}
print(grid_svc.best_score_)
# 0.8585810485810486

# print prediction results
prediction = grid_svc.predict(testX)
